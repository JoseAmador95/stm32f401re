...
void GPIO_Init(GPIO_Handle_t *pGPIOHandle)
{
	...
	if (pGPIOHandle->GPIO_PinConfig.GPIO_PinMode <= GPIO_MODE_ANALOG)
	{
		...
		pGPIOHandle->pGPIOx->MODER &= ~(3U << (2U * pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber));
	...
	else
	{
		...
		temp = PortCode << (4 * (pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber % 4));
		SYSCFG_PCLK_EN();
	...
	pGPIOHandle->pGPIOx->OSPEEDR &= ~(3U << (2 * pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber));
	pGPIOHandle->pGPIOx->OSPEEDR |= temp;

	temp = pGPIOHandle->GPIO_PinConfig.GPIO_pinPuPdControl << (2 * pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber);
	pGPIOHandle->pGPIOx->PUPDR &= ~(3U << (2 * pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber));
	...
	if (pGPIOHandle->GPIO_PinConfig.GPIO_PinMode == GPIO_MODE_ALTFN)
	{
		...
		pGPIOHandle->pGPIOx->AFR[pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber / 8] &= ~(0xFU << (4 * pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber));
...
void GPIO_WriteToOutputPin(typedef_GPIO_t *pGPIOx, uint8_t PinNumber, uint8_t Value)
{
...
{
	return GPIO_ReadFromInputPin(gpio->pGPIOx, gpio->GPIO_PinConfig.GPIO_PinNumber);
}

void GPIO_Write(GPIO_Handle_t* gpio, uint8_t value)
{
	GPIO_WriteToOutputPin(gpio->pGPIOx, gpio->GPIO_PinConfig.GPIO_PinNumber, value);
}
